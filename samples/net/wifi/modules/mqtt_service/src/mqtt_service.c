...
#ifdef CONFIG_MQTT_SERVICE_USE_IPV6
#define MQTT_SERVICE_IP_ADDR_STRING_LEN NET_IPV6_ADDR_LEN
#else
#define MQTT_SERVICE_IP_ADDR_STRING_LEN NET_IPV4_ADDR_LEN
#endif /* CONFIG_MQTT_SERVICE_USE_IPV6 */
...
int32_t mqtt_service_subscribe(const uint8_t *p_topic, uint32_t topic_len,
    uint8_t qos)
{
    int32_t err;
...
static int broker_init(void)
{
	...
	err = zsock_getaddrinfo(CONFIG_MQTT_SERVICE_SERVER_DOMAIN_NAME,
			  NULL, &hints, &result);
	if (err)
    {
...
static int32_t mqtt_service_client_init(struct mqtt_client *client)
{
    ...
    // 12 UID bytes (represented in hex) * 2 (one byte = 2 hex characters) + string null termination
    ...
    // Convert device serial number from binary (byte) to hex string representation
    ...
    case MQTT_EVT_PUBLISH: {
        ...
        while(len > 0)
        {
            bytes_read = mqtt_read_publish_payload(
                &client_ctx, payload, len > sizeof(payload) ? sizeof(payload) : len);
...
K_THREAD_DEFINE(mqtt_poll_thread, CONFIG_MQTT_SERVICE_THREAD_STACK_SIZE,
		mqtt_service_poll, NULL, NULL, NULL,
